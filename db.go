...
func getRows(db *sql.DB, d string, u string, p string) []*parser.DBRow {
	...
	// p = project (gotex)
	logger.Printf("Grabbing rows for %v/%v/%v.\n", d, u, p)
...
func addRows(db *sql.DB, c chan []*parser.DBRow) {
	...
	for i, r := range h {
		...
		if _, err := os.Stat(fmt.Sprintf("builds/%v/%v/%v.pdf", r.Path, r.ID, r.TeXRoot)); err == nil {
			...
			logger.Printf("File builds/%v/%v/%v.pdf doesn't exist. Maybe compilation failed.\n", r.Path, r.ID, r.TeXRoot)
		}
		// Diff PDF
		if _, err := os.Stat(fmt.Sprintf("builds/%v/%v/%v.diff.pdf", r.Path, r.ID, r.TeXRoot)); err == nil {
			...
			logger.Printf("File builds/%v/%v/%v.diff.pdf doesn't exist. Maybe compilation failed.\n", r.Path, r.ID, r.TeXRoot)
		}
		// Log PDF
		if _, err := os.Stat(fmt.Sprintf("builds/%v/%v/%v.log", r.Path, r.ID, r.TeXRoot)); err == nil {
			...
			logger.Printf("File builds/%v/%v/%v.log doesn't exist. Maybe compilation failed.\n", r.Path, r.ID, r.TeXRoot)
		}

		commitNumber += 1
		//logger.Println("Working through commit #", commitNumber)
		...
		if rows.Next() {
			...
			logger.Printf("I've already added row with commit hash %v to the database.\n", r.ID)
	...
	logger.Printf("Added %v rows to the database.\n", len(h))
}
